        ...
        where TPhenome : class
    {
        ...
        private IEnumerator EvaluateList(IList<TGenome> genomeList)
        {
            ...
            for (int i = 0; i < _neatSupervisor.Trials; i++)
            {
                ...
                while (true)
                {
                    if ((startTime - Time.time) > _neatSupervisor.TrialDuration)
                    {
                        break;
                    }
                    else
                    {
                        bool allDone = true;
                        foreach (TPhenome phenome in dict.Values)
                        {
                            UnitController controller = _neatSupervisor.GetController((IBlackBox)phenome);
                            if (controller != null)
                            {
                                allDone &= !controller.IsActive;
                            }
                        }
